...
class ProductBloc extends Bloc<ProductEvent, ProductState> {
  ...
  ProductBloc({
    ...
    on<LoadProduct>((event, emit) async {
      ...
      result.fold((failure) => emit(ProductError()), (_products) {
        Urls.allProducts = _products;
        emit(ProductLoaded(products: _products));
      });
    });

    on<FilteredProductEvent>(
      (event, emit) async {
        final filteredProduct = Urls.allProducts.where((product) {
    ...
    on<CreateProductEvent>((event, emit) async {
      emit(ProductLoading());
      final result =
          await insertProductUsecase.call(CreateParams(product: event.product));
      result.fold(
        (failure) => emit(
    ...
    on<UpdateProductEvent>((event, emit) async {
      emit(ProductLoading());
      final result =
          await updateProductUsecase.call(UpdateParams(product: event.product));
      result.fold(
        (failure) => emit(
